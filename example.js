...
module.exports = (bot) => {
  ...
  bot.respond(/reverse (.*)/i, function(res) {
   const reverseWord = res.match[1]
   console.log(reverseWord);
  var reversed =  reverseString(reverseWord);
   console.log(reversed)
        console.log(“reverse is working good”)
  ...
  var foods = [‘Burger’,‘Pasta’,‘Rice’];
     var favFood = foods[Math.floor(Math.random() * foods.length)];

     res.send(favFood);
 })

  bot.hear(/Chicken/i, function(res) {
   res.send('Al Baik!')
 })

  // Work end

  bot.respond(/What's your favorite team?/, function(res) {
    return res.send("I'm a robot--I don't watch football!");
  });
